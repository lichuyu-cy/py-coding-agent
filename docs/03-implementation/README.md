# 实现说明记录�?3-implementation�?

本目录保存每个开发阶段的实现说明 `NN-阶段�?md`，与实现、测试在同一 Git commit 中提交�?

- 阶段序号与命名对应设计文�?`01-overview/dependency-graph.md` �?23 阶段（如
  `02-message-state.md`、`04-minimal-agent-loop.md`、`14-compaction.md`）�?
- 文档模板与编写规范见设计文档 `docs/03-implementation/README.md`�?3 个必填小节）�?
- 文件列表必须来自实际 diff；测试必须给出真实命令与结果；不得把尚未实现的能力写成已完成�?
- 设计文档位于工作区兄弟目�?`coding-agent-harness-design/`；本仓库只包含实现与实现记录�?
